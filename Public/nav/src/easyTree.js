...
(function ($) {
    $.fn.EasyTree = function (options) {
        ...
        var createInput = $('<div class="input-group"><div class="col-md-3"><input type="text"  name="name" class="form-control" placeholder="菜单路径"></div><div class="col-md-3"><input type="text" name="title" placeholder="菜单名称" class="form-control"></div><div class="col-md-2"><span class="input-group-btn"><button type="button" class="btn btn-default btn-success confirm"></button> </span><span class="input-group-btn"><button type="button" class="btn btn-default cancel"></button> </span></div> </div> ');
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        if ($(createInput).find('input[name=name]').val() === ''|| $(createInput).find('input[name=title]').val()==='')
                            return;
                        var node_id= $('.li_selected').attr('#id');
                        var level= $('.li_selected').attr('level');
                        $.ajax({
                            'url':module+'/AdminNav/addNav',
                            'type':'post',
                            'data':{'id':node_id,'name':$(createInput).find('input[name=name]').val(),'level':level,'title':$(createInput).find('input[name=title]').val(),'aa':$('.li_selected').attr('aa')},
                            async:true,
                            success:function (data) {
                                ...
                                if(data==true){
                                    ...
                                    var item = $('<li><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input[name=title]').val() + '</a> </span></lia>');
                                    ...
                                    $(createInput).find('input[name=title]').val('');
                                    ...
                                    layer.msg('更新菜单成功');
                                    var selected = getSelectedItems();
                                    var item = $('<li #id="'+data.id+'" level="'+data.level+'" aa="'+data.name+'" ><span><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + $(createInput).find('input[name=title]').val() + '</a> </span></li>');
                                    $(item).find(' > span > span').attr('title', options.i18n.collapseTip);
                                    $(item).find(' > span > a').attr('title', options.i18n.selectTip);
                                    if (selected.length <= 0) {
                                        $(easyTree).find(' > ul').append($(item));
                        ...
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {
                                if ($(editor).val() !== '') {
                                    var node_id= $('.li_selected').attr('#id');
                                    var level= $('.li_selected').attr('level');
                                    var aa= $('.li_selected').attr('aa');
                                    var text=$(editor).val().trim();
                                    $.ajax({
                                        'url':module+'/AdminNav/editNav',
                                        'type':'post',
                                        'data':{'id':node_id,'name':aa,'level':level,'title':text},
                                         async:true,
                                         success:function (data) {
                                            if(data==true){
                                                $(selected).find(' > span > a').text(text);
                                                $(editor).remove();
                ...
                $(easyTree).find('.easy-tree-toolbar .remove > button').attr('title', options.i18n.deleteTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                            ...
                            var level= $('.li_selected').attr('level');
                            $.ajax({
                                'url':module+'/AdminNav/deleteNav',
                                'type':'post',
                                'data':{'id':node_id,'name':aa},
                                async:true,
                                success:function (data) {
                                   if(data==true){
                                       layer.msg('菜单删除成功');
